...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //String of str1 and str2 with str2 starting at where the overlap ends
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //finds all the strings in strSet that are substrings of str
        Set<String> substrings = new Set1L<>();
        for (String s : strSet) {
            if (str.contains(s)) {
                substrings.add(s);
            }
        ...
        //remove all the substrings from strSet
        for (String s : substrings) {
            strSet.remove(s);
        }

        //checks if str is in set
        boolean canAdd = true;
        for (String s : strSet) {
            if (s.contains(str)) {
                canAdd = false;
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //creates set to store result
        Set<String> result = new Set1L<>();

        while (!input.atEOS()) {
            String line = input.nextLine();
            addToSetAvoidingSubstrings(result, line);
        }
