...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (!p.isZero()) {
            ...
            nCopy.copyFrom(n);
            NaturalNumber remain = p.divide(two);
            powerMod(n, p, m);
            p.multiply(two);
            p.add(remain);
            NaturalNumber nSquare = n.newInstance();
            nSquare.copyFrom(n);
            n.multiply(nSquare);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber wCopy1 = new NaturalNumber2();
        NaturalNumber wCopy2 = new NaturalNumber2();
        wCopy1.copyFrom(w);
        wCopy2.copyFrom(w);

        powerMod(wCopy1, two, n);
        boolean rule1 = wCopy1.equals(one);

        NaturalNumber nSubOne = new NaturalNumber2();
        nSubOne.copyFrom(n);
        nSubOne.decrement();

        powerMod(wCopy2, nSubOne, n);
        boolean rule2 = wCopy2.compareTo(one) != 0;
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        // TODO - fill in body
        boolean flag = false;
        NaturalNumber nSubFour = new NaturalNumber2();
        nSubFour.copyFrom(n);
        nSubFour.subtract(new NaturalNumber2(4));
        for (int i = 0; i < 50 && flag == false; i++) {
            NaturalNumber wCanidate = new NaturalNumber2();
            wCanidate = randomNumber(nSubFour);
            wCanidate.add(new NaturalNumber2(2));
            flag = isWitnessToCompositeness(wCanidate, n);
        }
        return !flag;
